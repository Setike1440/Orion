...
export const GameCard: React.FC<GameCardProps> = ({ game, isLoading }) => {
  ...
  const getBadgeText = () => {
    ...
    if (!text || text === 'Garantia Vitalícia' || text === 'Garantia' || text === 'GARANTIA' || text === 'Garantia vitalícia' || text === 'Jogue Agora' || text === 'Jogar Agora') {
      return t('play_now');
    }
    return text;
  };
  ...
  if (isLoading || !game) {
    return (
      <div className="bg-[#0d0e12] border border-[#1f212a] rounded-2xl p-3 flex flex-col gap-3">
  ...
  return (
    <Link 
      to={`/jogo/${game.id}`}
      className="group bg-[#0d0e12] border border-[#1f212a] hover:border-[#FF0000]/40 rounded-2xl p-3 flex flex-col gap-3 transition-all duration-200 relative overflow-hidden hover:-translate-y-1 hover:shadow-xl cursor-pointer"
    >
      <div className="relative aspect-[460/215] rounded-xl overflow-hidden bg-[#171821]">
        ...
        <div className="absolute top-2 left-2 bg-[#FF0000] text-white text-[10px] font-semibold px-2 py-0.5 rounded-md flex items-center gap-1 shadow-sm z-10">
        ...
        <button 
          ...
          className={cn(
            ...
            isFavorite 
              ...
              : "bg-[#000000]/70 text-gray-300 hover:text-white hover:scale-105 border border-white/10"
          )}
      ...
      <div className="px-0.5 flex flex-col flex-1">
        <h3 className="text-white font-medium text-xs sm:text-sm truncate block leading-snug mb-0.5 group-hover:text-[#FF0000] transition-colors">
          {game.title}
        </h3>
